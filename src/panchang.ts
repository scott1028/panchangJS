...
var Lmoon: any, Lsun: any, skor: any, LmoonYoga: any, LsunYoga: any, dt: any, kyear: any, kday: any, kmon: any;
var ayanamsa = 0;
...
//---------------------------------------------------------------------------
// function corr(mlcor: any, mscor: any, fcor: any, dcor: any, lcor: any) {
//     this.mlcor = mlcor;
//     this.mscor = mscor;
//     this.fcor = fcor;
//     this.dcor = dcor;
...
// function corr2(l: any, ml: any, ms: any, f: any, d: any) {
//     this.l = l;
//     this.ml = ml;
//     this.ms = ms;
//     this.f = f;
...
function daysInMonth(m: any, y: any) {
...
function moon(jd: any) {
    ...
    for (i = 0; i < 93; i++) { // outrage at length
        var arg = corrMoon[i].mlcor * ml + corrMoon[i].mscor * ms + corrMoon[i].fcor * f + corrMoon[i].dcor * d;
    ...
    for (i = 0; i < 27; i++) { // outrage at length additional
        var arg = corrMoon2[i].ml * ml + corrMoon2[i].ms * ms + corrMoon2[i].f * f + corrMoon2[i].d * d;
        var sinarg = Math.sin(arg);
...
function sun(jd: any) {
    ...
    var g = ms + (0.266 * Math.sin((31.8 + 119.0 * t) * d2r) + 6.40 * Math.sin((231.19 + 20.2 * t) * d2r) + (1.882 - 0.016 * t) * Math.sin((57.24 + 150.27 * t) * d2r)) / 3600.0;
    // Rising sun node len
    ...
    var truanom = fix360(truanom * r2d);
...
//----------------------------------------------------------------------------
function tithi(jd: any, n1: any, tzone: any, len: any) {
    var s_t: { start: any, end: any } = { start: undefined, end: undefined };
    var flag;
    var jdt = jd;
...
//----------------------------------------------------------------------------
function nakshatra(jd: any, n_naksh: any, tzone: any) {
    var s_t: { start: any, end: any } = { start: undefined, end: undefined };
    var flag;
    var jdt = jd;
    ...
    for (var inak = n_naksh; inak < (n_naksh + 2); ++inak) {
        ...
        flag = 0;
        while (flag < 1) {
            var Lmoon0 = fix360(moon(jdt) + ayanamsa);
...
//----------------------------------------------------------------------------
function yoga(jd: any, zyoga: any, tzone: any) {
    var s_t: { start: any, end: any } = { start: undefined, end: undefined };
    var flag;
    var jdt = jd;
    var z = zyoga;
...
//-----------------------------------------------------------------------------
function novolun(jd: any, knv: any) {
...
//-----------------------------------------------------
function kepler(m: any, ex: any, err: any) {
    //val u0, delta;
...
function calData(jd: any) {
    ...
    if (z2 < 2299161) a = z2;
        ...
        var alf = Math.floor((z2 - 1867216.25) / 36524.25);
        var a = z2 + 1 + alf - Math.floor(alf / 4);
    }
...
export var panchang = {
    ...
    Raasi: {} as any,
    version: "0.2",
    calculate: function (d: any, cb: any) {
        var n_wday, n_tithi = 1,
            n_naksh = 1,
            n_karana, n_yoga;
        ...
        // }
        // no use?
        // var inpmin: number, string = Math.floor(d.getMinutes());
        ...
        // Nakstra
        var Lmoon0 = fix360(Lmoon + ayanamsa);
        ...
        // with(Math) {
        var z = Math.floor(Math.abs(fix360(Lmoon + ayanamsa)) / 30);
        // }
        ...
        this.Karna.name = kar[n_karana as number];
