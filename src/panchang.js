...
function sun(jd) {
    ...
    // with(Math) {
        ...
        r1 = 1.0000002 * (1 - ex * ex) / (1 + ex * Math.cos(truanom * d2r));
...
function tithi(jd, n1, tzone, len) {
    ...
    for (itit = n1; itit < (n1 + 2); ++itit) {
        ...
        flag = 0;
        if (aspect == 0) { jdt = novolun(jd, knv);
            flag = 1; }
        if (aspect == 360) { jdt = novolun(jd, (knv + 1));
            flag = 1; }
        while (flag < 1) {
            Lsun0 = sun(jdt);
            Lmoon0 = moon(jdt);
            ...
            if (Math.abs(asp1) > 0.001) { jdt += (asp1 / (skor - 1));
                flag = 0; }
        }
...
function nakshatra(jd, n_naksh, tzone) {
    ...
    for (inak = n_naksh; inak < (n_naksh + 2); ++inak) {
        ...
        while (flag < 1) {
            ...
            if (asp1 < -180) asp1 += 360;
            flag = 1;
            if (Math.abs(asp1) > 0.001) { jdt += (asp1 / skor);
                flag = 0; }
        }
...
function yoga(jd, zyoga, tzone) {
    ...
    for (iyog = 0; iyog < 2; ++iyog) {
        ...
        while (flag < 1) {
            ...
            asp1 = nn_yoga[iyog] - z;
            flag = 1;
            if (Math.abs(asp1) > 0.001) { jdt += (asp1 / (skor + 1.0145616633));
                flag = 0; }
